...
public class BackpackBEMenu extends BackpackInfoMenu {
    ...
    public BackpackBEMenu(int id, Inventory player, FriendlyByteBuf buf) {
        this(id, player, new SimpleContainer(buf.readInt()), BlockPos.ZERO, buf.readInt(), buf.readInt());
    }

    public BackpackBEMenu(int id, Inventory player, Container backpackInventory, BlockPos pos, int rows, int columns) {
        super(MenuInit.BACKPACK_BE_MENU.get(), id);
        this.rows = rows;
